...
# Matches function argument names. The lookahead requires a ':' or '=' so that
# generic types like `Array[String]` are ignored.
PARAM_NAME_RE = re.compile(r"(?:^|,)\s*([A-Za-z_][A-Za-z0-9_]*)\s*(?=[:=])")
        ...
        if fname.endswith('.gd'):
            ...
            for idx, line in enumerate(lines, 1):
                ...
                if m:
                    ...
                    if params:
                        params = params.strip('()')
                        for param_name in PARAM_NAME_RE.findall(params):
                            if not SNAKE_CASE.match(param_name):
                ...
                m = re.match(r"const\s+([A-Za-z_][A-Za-z0-9_]*)", stripped)
                if m:
                    name = m.group(1)
